...
# 2. Done - In each role policy, under identifier add only the users that have that
#    Specific listed in their "roles" list
#
...
data "aws_iam_policy_document" "assume_role_policy" {
  ...
  statement {
    ...
    principals {
      type = "AWS"
      identifiers = [
        for username in keys(aws_iam_user.users) : "arn:aws:iam::${data.aws_caller_identity.current.id}:user/${username}"
